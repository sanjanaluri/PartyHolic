...
func GetParties(c *gin.Context) {
	...
	distance_string := "select p.Party_name, concat_ws(' ', u.first_name, u.last_name) as Host_name, p.attendee_count, round(" + distance_calculation + ",2) as Distance, p.image_id from parties p join users u on p.host_id = u.user_id order by 4 limit 20"
